    ...
    <div className="page">
      ...
      <div className="settings-grid">
        ...
        <section className="card settings-panel">
          ...
          {section === "Data sources" && <><h2>{t("Data sources")}</h2><p className="muted">{t("Eight interchangeable public connectors are available server-side. This browser workspace remains mock-first until the server enables them and the user explicitly consents; RSS also requires administrator-selected feeds.")}</p>{providerRegistry.dataSources.map((provider) => <div className="setting-row" key={provider.id}><div className="provider-status"><span className={`dot ${provider.status === "active" ? "active" : ""}`} /><div><strong>{t(provider.label)}</strong><p>{t(provider.status)}</p></div></div>{provider.status === "active" ? <span className="badge green">{t("Default")}</span> : provider.status.startsWith("available") ? <span className="badge orange">{t("Available")}</span> : <span className="badge">{t("Offline")}</span>}</div>)}</>}
